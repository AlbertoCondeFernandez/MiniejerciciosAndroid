...
data class UsuarioData(
    @PrimaryKey(autoGenerate = true) @ColumnInfo(name = Estructura.Usuario.IDUSUARIO) val idUsuario: Int = 0, // Clave primaria autogenerada automáticamente, generando valores únicos incrementales. El indicar que toma el valor 0 permite que cuando se cree un objeto, no sea necesario asignar manualmente un valor.
...
@Entity( // Marca la clase como una entidad que corresponde a una tabla en la BD, accediendo por el nombre. Además, se crea la clave foránea con foreignKeys.
    ...
    foreignKeys = [ForeignKey(
        ...
        onDelete = ForeignKey.CASCADE // Si se elimina un usuario en la tabla UsuarioData, todas las sesiones relacionadas en la tabla de sesiones también se eliminarán automáticamente (borrado en cascada).
    )]
)
